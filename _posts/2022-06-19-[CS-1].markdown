...
사용자의 관점에서 기능에 영향을 미치지 않고 서브 클래스를 부모 클래스로 대체 할 수 있어야 함.
...
-------
근데 이렇게 이론적으로 쓰면 너무 뜬구름 잡는 내용으로 보인다.

사실 리스코프 치환 원칙은 인터페이스, 추상클래스, 다형성, 상속 등 객체지향의 두드러지는 특징을 적절하게 사용하게끔 하는 원칙이다.

아래는 리스코프 치환 원칙을 이해하기 위한 여러 특징이다.
그나마 실무에 활용 가능하고 와닿게 얘기하자면 다음과 같다.
...
인터페이스와 추상 클래스의 차이는 무엇일까? 둘 다 상속한다는 공통점을 가지고 있다.
인터페이스를 상속받는 것은 마치 "너 이런 방식대로 행동하게 만들어져야돼"라고 하는 느낌이다.
그러나 추상 클래스는 "너 이런 방식대로 만들어. 대신 확장은 마음대로 해"라고 하는 느낌.

왜 이런 느낌을 받냐면, 클래스는 다중상속이 안되고 인터페이스는 다중상속이 가능하다.

확장을 위한 것은 추상 클래스, 클래스의 규격을 만드는 것은 인터페이스.
...
- 이력 제약 조건 (이력 규칙). 객체는 그 자신의 메서드를 통해서만 수정(캡슐화)할 수 있는 것으로 간주된다.
